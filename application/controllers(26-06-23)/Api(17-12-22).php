	...
	{  
		...
		if($result['status']==1)
		{
			   $userid= $result['data']->userid;
				...
				$enddate1 = date('H:i:A',strtotime('23:50:00'));   // Second End date
			    $ct =  date('H:i:A');  
				...
				$liveroute_details=$this->api_model->livetrackvehiclelist($userid,$shift_type);
						 ...
						 $stoplatlng = $this->api_model->stoplatlng($stop_geoid);
						  ...
						  $distance = $this->geo_distance($stop_lat,$stoplng,$vehicle_lat,$vehicle_lng);
							...
							$stop_status = ($stopentry_time==null && $stopexit_time==null) ?  "Not Reached" :'Reached';
                                            ...
                                            $livestop_data[] = array('stop_name' =>$stoplist->stop_name,
