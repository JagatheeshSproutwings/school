...
<div class="content-right" style="width: calc(100% - 350px) !important;">
			...
			<div class="card">
                            <ul class="nav nav-tabs nav-topline" role="tablist">
                                    ...
                                    <a class="nav-link pull-right" href="<?php echo site_url('Dashboard/google_mapview');?>">Google Map</a>
...
<script type="text/javascript">
      ...
      function single_vehicle_route(map,data)
    {
      ...
      var playback_polyline = [];
    var playback_data = data.playback_data;
    if ((playback_data) != '') {
      $.each(data.playback_data, function(key, value) {
        playback_polyline.push([value.lat_message, value.lon_message]);
      });
    }
      $("#pre_lat").empty();
         $("#pre_lng").empty();
...
//alert(data.mdvr_terminal_no);
  ...
  $.ajax({
         ...
         function single_vehicle_route1(map,data)
    {
      ...
      var playback_polyline = [];
      var playback_data = data.playback_data;
      if(playback_polyline != undefined)
      {
        map.removeLayer(playback_polyline);
        if((playback_data)!=''){
          $.each(data.playback_data,function(key,value){
              playback_polyline.push([value.lat_message,value.lon_message]);
          });
      ...
      console.log("Play Back :" +playback_polyline);
      if (marker != undefined) {
              map.removeLayer(marker);
            };
