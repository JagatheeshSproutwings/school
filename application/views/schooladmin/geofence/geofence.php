                            ...
                            <div class="card">
                                ...
                                <button type="button" class="btn btn-primary btn-min-width" id="closeform">Reset</button>
                                               ...
                                               <div id="map" style="height:500px;"></div>
												...
												<hr>
												<!-- FORM - IMPORT GEO FENCE -->
												
												<div class="col-md-12">
													<h4 class="text-center">Bulk Import </h4>
													<div class="col-md-4">
															<div class="form-group">
															<label for="">Step 1: Download Sample</label>	
															<a class="btn btn-sm btn-success" href="<?=base_url('assets/geo_fence/geo_fence.xlsx')?>" download="">Download</a>	
															</div>
													</div>
													<form id="initialize_geo_fence" method="POST" enctype="multipart/form-data">	
													<div class="col-md-4">
															<div class="form-group">
															<label for="">Step 2: Upload File</label>	
																	<input type="file" name="geofence_file" class="form-control">
															</div>
													</div>
													<input type="hidden" name="user_id" value="<?=$user_id?$user_id:NULL;?>">
													<input type="hidden" name="dealer_id" value="<?=$dealer_id?$dealer_id:NULL;?>">
													<input type="hidden" name="client_id" value="<?=$client_id?$client_id:NULL;?>">
													<div class="col-md-4 col-offset-md-8 py-5">
															<button type="submit" class="btn btn-primary text-center" name="geofence_upload">upload</button>
													</div>
													</form>
												</div>
												<!-- FORM - IMPORT GEO FENCE -->
    ...
    else
    {
     ...
     $('.adduser').show(2000);  //Add userpage hide
      ...
      location.reload(); 
		 ...
		 $("#import_geofence").click(function(e){
			e.preventDefault();
			$.ajax({
				url:"<?=base_url('geofence/import_geofence')?>",
				type:"POST",
				data:{test:'test'},
				success:function(response)
				{
		 ...
		 $("#initialize_geo_fence").on('submit',(function(e) 
        {
					e.preventDefault();
					$.ajax({
						url:"<?=?>",
