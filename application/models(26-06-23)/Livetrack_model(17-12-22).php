...
{	
	function __construct()
	{
		...
		public function livetrackvehicles($shift_type,$timezone_minute) 
	{	
		$client_id = $this->session->userdata['client_id'];
		if($client_id=='')
		{
			$query = $this->db->query("SELECT ra.*,sr.route_name,sr.route_geo_start_id,l1.Lat as routestart_lat,l1.Lng as routestart_lng,l1.radius as routestart_radius,l2.radius as routeend_radius,sr.route_geo_end_id,l2.Lat as routeend_lat,l2.Lng as routeend_lng,sr.route_starttime,sr.route_endtime,v.vehiclename,v.vehicleid,v.lat,v.lng,v.speed,v.acc_on FROM `sch_routeassigntbl` ra INNER JOIN sch_routestbl sr ON sr.route_id = ra.route_id INNER JOIN vehicletbl v ON v.vehicleid=ra.vehicle_id INNER JOIN sch_location_list l1 ON l1.Location_Id=sr.route_geo_start_id INNER JOIN sch_location_list l2 ON l2.Location_Id=sr.route_geo_end_id WHERE ra.`shift_type`=$shift_type AND v.timezone_minutes=$timezone_minute");
		

		}
		else
		{
			$query = $this->db->query("SELECT ra.*,sr.route_name,sr.route_geo_start_id,l1.Lat as routestart_lat,l1.Lng as routestart_lng,l1.radius as routestart_radius,l2.radius as routeend_radius,sr.route_geo_end_id,l2.Lat as routeend_lat,l2.Lng as routeend_lng,sr.route_starttime,sr.route_endtime,v.vehiclename,v.vehicleid,v.lat,v.lng,v.speed,v.acc_on FROM `sch_routeassigntbl` ra INNER JOIN sch_routestbl sr ON sr.route_id = ra.route_id INNER JOIN vehicletbl v ON v.vehicleid=ra.vehicle_id INNER JOIN sch_location_list l1 ON l1.Location_Id=sr.route_geo_start_id INNER JOIN sch_location_list l2 ON l2.Location_Id=sr.route_geo_end_id WHERE ra.`shift_type`=$shift_type AND sr.client_id=$client_id AND v.timezone_minutes=$timezone_minute");
...
{		
	...
	{
		$query = $this->db->query("SELECT ss.*,r.route_name FROM sch_student ss INNER JOIN sch_routestbl r ON r.route_id=ss.route_id INNER JOIN usertbl u ON u.userid=ss.parent_id
	WHERE ss.route_id=$route_id AND (ss.status=2 OR ss.status=3)");
...
public function stopstart_alert($route_id,$stop_id) 
{		
