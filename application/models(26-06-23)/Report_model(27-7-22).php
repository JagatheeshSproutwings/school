...
// echo "select geo_location_id,vehicle_id,in_datetime from trip_geo_solution_data where geo_location_id ='".$stop_geo_id."' and vehicle_id = '".$vehicle_id."' and DATE_FORMAT(in_datetime, '%Y-%m-%d %H:%i:%s') BETWEEN '".$first_start_time."' AND '".$end_end_time."'";exit;
	...
	public function liveroute_list($travel_date)
	{
		
		    $client_id = $this->session->userdata['client_id'];
			$query1 = $this->db->query("SELECT al.*,TIMESTAMPDIFF(MINUTE,al.route_planstart_time,al.route_exitime) as start_min,TIMESTAMPDIFF(MINUTE,al.route_planend_time,al.route_entry_time) as end_min FROM sch_add_liveroute_list al WHERE travel_date='$travel_date' AND client_id=$client_id");	
