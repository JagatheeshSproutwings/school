...
class Dashboardmodel extends CI_Model
{
  ...
  public function vehicle_details()
     ...
     public function all_vehicle_loc($v_status=null)
    {
        ...
        vehicletype as vehicle_type,angle,vehicleid,vehiclename,lat,lng,speed,(updatedon + INTERVAL $timezone_hours MINUTE) as updatedon,TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) AS update_time,lat,lng,
        ...
        elseif ($v_status=='2') 
        {
           ...
           $query = $this->db->query("SELECT simnumber,installationdate,expiredate,mdvr_terminal_no,today_km,internal_battery_voltage,vehicle_sleep,deviceimei,angle,
           vehicletype as vehicle_type,angle,vehicleid,vehiclename,lat,lng,speed,(updatedon + INTERVAL $timezone_hours MINUTE) as updatedon,TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) AS update_time,lat,lng,acc_on,
        ...
        $query = $this->db->query("SELECT simnumber,installationdate,expiredate,mdvr_terminal_no,today_km,internal_battery_voltage,vehicle_sleep,deviceimei,angle,
        vehicletype as vehicle_type,angle,vehicleid,vehiclename,lat,lng,speed,(updatedon + INTERVAL $timezone_hours MINUTE) as updatedon,TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) AS update_time,lat,lng,acc_on,acc_flag,
       ...
       public function alert_data($deviceimei)
    {
      ...
      $query =  $this->db->query("SELECT ta.createdon,att.alert_type FROM sms_alert ta LEFT JOIN alert_type att ON ta.all_status = att.alert_type_id INNER JOIN vehicletbl v ON v.vehicleid = ta.vehicle_id WHERE ta.client_id = '".$client_id."' AND v.deviceimei='".$deviceimei."' ORDER BY ta.sms_alert_id DESC LIMIT 10");
