...
class Client_model extends CI_model
{
   function __construct()
	...
	public function all_client_data() { 
		...
		$query=$this->db->query("SELECT c.*,u.username FROM clienttbl c INNER JOIN usertbl u ON u.client_id=c.client_id WHERE c.dealer_id ='".$dealer_id."' AND (c.subdealer_id =0 OR c.subdealer_id IS NULL)");
		}
		if ($query->num_rows () > 0) {
			return $query->result ();
		} else {
