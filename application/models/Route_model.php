...
{	
	...
	$query = $this->db->query("SELECT count(*) as student_count FROM sch_student WHERE (route_id=$route_id OR evening_route_id=$route_id) AND (stop_id = $stop_id OR evening_stop_id=$stop_id)");
	if ($query->num_rows() > 0) 
	{
		$data =  $query->row();
...
{		
	$query = $this->db->query("SELECT stu.student_name,stu.student_rollno,sc.class_name,ss.section_name FROM sch_student stu 
	INNER JOIN sch_class sc ON sc.class_id = stu.class_id
	INNER JOIN sch_section ss ON ss.section_id = stu.section_id
	WHERE (stu.route_id=$route_id OR stu.evening_route_id=$route_id) AND (stu.stop_id=$stop_id OR stu.evening_stop_id=$stop_id)");
...
{
	$query = $this->db->query("SELECT CONCAT(firstname,'-' ,lastname) AS parent_name,stu.stop_id,ut.mobilenumber,ut.alter_mobile,sll.Location_short_name,sll.Lat,sll.Lng,sr.route_name,srt.stop_arrival_time,sc.class_name,stu.* FROM sch_student stu 
	...
	INNER JOIN sch_class sc ON sc.class_id=stu.class_id
	Where (stu.route_id=$id OR stu.evening_route_id=$id) ORDER BY srt.stop_arrival_time ASC,stu.stop_id ASC");
...
{	
	...
	INNER JOIN sch_location_list sll ON sll.Location_Id=srt.stop_geo_id
	WHERE 
	(stu.route_id=$route_id OR stu.evening_route_id=$route_id) AND (stu.stop_id=$stop_id OR stu.evening_stop_id =$stop_id)");
