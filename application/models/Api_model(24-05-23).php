...
class Api_model extends CI_model
{
   function __construct()
	...
	public function student_details ($userid)
	{		
		 $query=$this->db->query("SELECT route_id,evening_route_id,evening_stop_id,stop_id,student_name,student_rollno,class_id,section_id FROM sch_student WHERE parent_id =$userid" );
		if($query->num_rows() > 0)
	...
	{		
        ...
        srt.route_geo_start_id,
        l.Location_short_name AS start_loc_name,
        l.Lat AS routestartlat,
        l.Lng AS routestartlng,
        l1.Location_short_name AS end_loc_name,
        l1.Lat AS routeendlat,
        l1.Lng AS routeendlng,
        srt.route_geo_end_id,
        srt.route_starttime,
        srt.route_endtime,
        v.vehiclename,
        v.deviceimei,
      ...
      FROM
        sch_student ss
      LEFT JOIN
        sch_routeassigntbl sr ON sr.route_id = ss.route_id
      LEFT JOIN
        sch_routestbl srt ON srt.route_id = ss.route_id
      LEFT JOIN
        sch_routestbl srt1 ON srt.route_id = ss.evening_route_id
      INNER JOIN
        vehicletbl_2 v ON sr.vehicle_id = v.vehicleid
      INNER JOIN
        sch_location_list l ON l.Location_Id = srt.route_geo_start_id
      INNER JOIN
        sch_location_list l1 ON l1.Location_Id = srt.route_geo_end_id WHERE ss.parent_id =$userid GROUP BY ss.student_id");
    
		//  $query=$this->db->query("SELECT sr.shift_type,ss.evening_route_id,ss.evening_stop_id,ss.student_id,ss.student_name,ss.student_rollno,
        //  ss.parent_id,ss.stop_id,ss.route_id,ss.class_id,ss.section_id,srt.route_name,v.vehicleid as vehicle_id,srt.route_geo_start_id,
        ...
        //   FROM sch_student ss LEFT JOIN sch_routeassigntbl sr ON sr.route_id=ss.route_id INNER JOIN sch_routestbl srt
    ...
    public function get_shifttype($route_id) 
	{	
		
		$query = $this->db->query("SELECT ra.shift_type  FROM `sch_routeassigntbl` ra  WHERE ra.route_id=$route_id");
	...
	public function single_livetrackvehicles($route_id) 
	{	
		
		$query = $this->db->query("SELECT ra.*,sr.route_name,sr.route_geo_start_id,sr.route_starttime,sr.route_endtime,l1.Lat as routestartlat,
        l1.Lng as routestartlng,sr.route_geo_end_id,l2.Lat as routeendlat,l1.Location_short_name as start_loc_name,l2.Location_short_name as end_loc_name,
        l2.Lng as routeendlng,sr.route_starttime,sr.route_endtime,v.vehiclename,v.vehicleid as vehicle_id,v.lat,v.lng,v.speed,v.acc_on,v.deviceimei 
        FROM `sch_routeassigntbl` ra INNER JOIN sch_routestbl sr ON sr.route_id = ra.route_id INNER JOIN vehicletbl v ON 
        v.vehicleid=ra.vehicle_id INNER JOIN sch_location_list l1 ON l1.Location_Id=sr.route_geo_start_id INNER JOIN
         sch_location_list l2 ON l2.Location_Id=sr.route_geo_end_id WHERE ra.route_id=$route_id");
...
public function checkuser_admin($username,$password)
	{		
		 ...
		 $query=$this->db->query("SELECT userid,firstname,username,companyname,mobilenumber,email,roleid,client_id,dealer_id,subdealer_id,push_code,time_zone FROM usertbl WHERE status=1 AND roleid='15' AND username='$username' AND (password='$password' OR secondarypassword='$password')" );
	...
	{		
		 $query=$this->db->query("SELECT u.userid,u.firstname,u.username,u.companyname,u.mobilenumber,u.email,u.roleid,u.client_id,u.dealer_id,u.subdealer_id,u.push_code,u.time_zone,tz.timezone_mins FROM usertbl u INNER JOIN time_zone tz ON tz.timezone_id=u.time_zone  WHERE status=1 AND roleid='15' AND userid=$userid" );
	...
	public function total_count($userid,$client_id,$roleid,$timezone_hours)
	{	
	    $query=$this->db->query("SELECT (SELECT count(v.vehicleid) FROM vehicletbl v  WHERE 
			v.status=1 AND TIMESTAMPDIFF(MINUTE,(v.updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) <= '10' AND v.acc_on =0 AND v.client_id=$client_id) as park_count,
			(SELECT count(v.vehicleid) FROM vehicletbl v WHERE v.status=1 AND 
			TIMESTAMPDIFF(MINUTE,(v.updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) <= '10' AND v.acc_on =1 and round(v.speed) = 0 and v.client_id=$client_id) as idle_count,
			(SELECT count(v.vehicleid) FROM vehicletbl v  WHERE v.status=1 AND 
			TIMESTAMPDIFF(MINUTE,(v.updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) <= '10' AND v.acc_on =1 and round(v.speed) >= 1 and v.client_id=$client_id) as move_count,
			(SELECT count(v.vehicleid) FROM vehicletbl v  WHERE (TIMESTAMPDIFF(MINUTE,(v.updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) > '10'
	...
	public function vehicledetails($deviceimei,$timezone_hours)
	{		

		 $query=$this->db->query("SELECT client_id,vehicleid as vehicle_id,(updatedon + INTERVAL $timezone_hours MINUTE) as updatedon,TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) AS update_time,
	...
	public function vehiclelist($client_id,$status,$timezone_hours)
	{	
       
        if($status==1)
		{
			$query=$this->db->query("SELECT IF(
				TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) <= '10' AND v.acc_on =1 and round(v.speed) >= 1,
				'Moving',
				IF(
				 TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) <= '10' AND v.acc_on =1 and round(v.speed) = 0 ,
				'Idle',
				IF(
				TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) <= '10' AND v.acc_on =0,         
					'Parking',          
					'OutOfCoverage'
				  )
				  )
				) as vehicle_status 
		  ,vehiclename,TIME_FORMAT(TIMEDIFF(CURRENT_TIMESTAMP,(last_ign_off + INTERVAL $timezone_hours MINUTE)), '%H Hours:%i Minutes') as last_update,
					  ...
					  ROUND(car_battery,2) as battery_voltage, acc_on as acc_status, ac_flag as ac_status,IF(car_battery<7, 0, 1) as battery_status,digital_output as immoblizer_status,gpssignal as gps_status,gsmsignal as gsmlevel,
                      lat,lng,deviceimei as vehicle_imei, 
					  TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) AS update_time,angle,vehicletype,(updatedon + INTERVAL $timezone_hours MINUTE) as updatedon,safe_parking 
			FROM vehicletbl v  WHERE v.status=1 AND v.client_id=$client_id" );
		}
		elseif($status==2)			
		{
			$query=$this->db->query("SELECT 'Moving' as vehicle_status,vehicleid,vehiclename,TIME_FORMAT(TIMEDIFF(CURRENT_TIMESTAMP,(last_ign_off + INTERVAL $timezone_hours MINUTE)), '%H Hours:%i Minutes') as last_update,COALESCE(driver_name,'N/A') as driver_name,
			...
			TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) AS update_time,angle,vehicletype,(updatedon + INTERVAL $timezone_hours MINUTE) as updatedon,safe_parking
			FROM vehicletbl v  WHERE v.status=1 AND TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) <= '10' AND v.acc_on =1 and round(v.speed) >= 1 and v.client_id=$client_id" );
		}	
		elseif($status==3)
		{
			$query=$this->db->query("SELECT 'Idle' as vehicle_status,vehicleid,vehiclename,TIME_FORMAT(TIMEDIFF(CURRENT_TIMESTAMP,(last_ign_off + INTERVAL $timezone_hours MINUTE)), '%H Hours:%i Minutes') as last_update,COALESCE(driver_name,'N/A') as driver_name,
			ROUND(speed) as speed,ROUND(odometer,3) as odometer,COALESCE(latlon_address,'') as address,ROUND(litres,2) as fuel,ROUND(car_battery,2) as battery_voltage,
			acc_on as acc_status,ac_flag as ac_status,IF(car_battery<7, 0, 1) as battery_status,digital_output as immoblizer_status,gpssignal as gps_status,gsmsignal as gsmlevel,lat,lng,deviceimei as vehicle_imei, 
			TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) AS update_time,angle,vehicletype,(updatedon + INTERVAL $timezone_hours MINUTE) as updatedon,safe_parking
			FROM vehicletbl v  WHERE v.status=1 AND TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) <= '10' AND v.acc_on =1 and round(v.speed) = 0 
			and	v.client_id=$client_id" );
		}	
		elseif($status==4)
		{
			$query=$this->db->query("SELECT 'Parking' as vehicle_status,vehicleid,vehiclename,TIME_FORMAT(TIMEDIFF(CURRENT_TIMESTAMP,(last_ign_off + INTERVAL $timezone_hours MINUTE)), '%H Hours:%i Minutes') as last_update,COALESCE(driver_name,'N/A') as driver_name,
			ROUND(speed) as speed,ROUND(odometer,3) as odometer,COALESCE(latlon_address,'') as address,ROUND(litres,2) as fuel,ROUND(car_battery,2) as battery_voltage,
			acc_on as acc_status,ac_flag as ac_status,IF(car_battery<7, 0, 1) as battery_status,digital_output as immoblizer_status,gpssignal as gps_status,gsmsignal as gsmlevel,lat,lng,deviceimei as vehicle_imei, 
			TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) AS update_time,angle,vehicletype,(updatedon + INTERVAL $timezone_hours MINUTE) as updatedon,safe_parking
			FROM vehicletbl v  WHERE v.status=1  AND TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) <= '10' AND v.acc_on =0 
			AND v.client_id=$client_id" );
		}	
		elseif($status==5)
		{
			$query=$this->db->query("SELECT 'OutOfCoverage' as vehicle_status,vehicleid,vehiclename,TIME_FORMAT(TIMEDIFF(CURRENT_TIMESTAMP,(last_ign_off + INTERVAL $timezone_hours MINUTE)), '%H Hours:%i Minutes') as last_update,
			COALESCE(driver_name,'N/A') as driver_name, ROUND(speed) as speed,ROUND(odometer,3) as odometer,
			COALESCE(latlon_address,'') as address,ROUND(litres,2) as fuel,ROUND(car_battery,2) as battery_voltage, 
			acc_on as acc_status,ac_flag as ac_status,IF(car_battery<7, 0, 1) as battery_status,digital_output as immoblizer_status,gpssignal as gps_status,gsmsignal as gsmlevel,lat,lng,deviceimei as vehicle_imei, 
            TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) AS update_time,
			angle,vehicletype,(updatedon + INTERVAL $timezone_hours MINUTE) as updatedon,safe_parking 
			FROM vehicletbl v WHERE v.status=1 AND (TIMESTAMPDIFF(MINUTE,(updatedon + INTERVAL $timezone_hours MINUTE),CURRENT_TIMESTAMP) > '10' OR updatedon IS NULL or acc_on IS NULL or speed IS NULL)
			AND v.client_id=$client_id" );
		}
		else{
			$query=$this->db->query("SELECT IF(
	...
	public function trip_report_list($from, $to, $vehicleid = NULL, $time, $timezone_hours)
    {
        ...
        if (!empty($vehicleid)) {
            ...
            $query1 = $this->db->query("SELECT TIME_FORMAT(TIMEDIFF((ip.end_day + INTERVAL $timezone_hours MINUTE),(ip.start_day + INTERVAL $timezone_hours MINUTE)), '%H:%i:%s') as time_duration,
                        ...
                        (ip.start_day + INTERVAL $timezone_hours MINUTE) as start_day,(ip.end_day + INTERVAL $timezone_hours MINUTE) as end_day,ROUND((ip.end_odometer - ip.start_odometer),2) as distance,
    ...
    public function playback_data_admin($d_id, $from, $to,$client_id,$timezone_hours)
    {
