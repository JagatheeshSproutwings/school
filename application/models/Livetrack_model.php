...
class Livetrack_model extends CI_Model
{
	...
	public function tripstart_alert($route_id, $checkid, $shift_type)
	{
		if ($checkid == 1) {
			if ($shift_type == 1) {
		...
		WHERE ss.route_id=$route_id AND ss.status=1");
				...
				$query = $this->db->query("SELECT ss.*,r.route_name FROM sch_student ss INNER JOIN sch_routestbl r ON r.route_id=ss.route_id INNER JOIN usertbl u ON u.userid=ss.parent_id
		    WHERE ss.evening_route_id=$route_id AND ss.status=1");
			}
		}
		...
		WHERE ss.route_id=$route_id AND (ss.status=2 OR ss.status=3 OR ss.status=5)");
				...
				$query = $this->db->query("SELECT ss.*,r.route_name FROM sch_student ss INNER JOIN sch_routestbl r ON r.route_id=ss.route_id INNER JOIN usertbl u ON u.userid=ss.parent_id
		    WHERE ss.evening_route_id=$route_id AND (ss.status=2 OR ss.status=3 OR ss.status=5)");
			}
		}
		...
		WHERE ss.route_id=$route_id AND ss.status=2");
				...
				$query = $this->db->query("SELECT ss.*,r.route_name FROM sch_student ss INNER JOIN sch_routestbl r ON r.route_id=ss.route_id INNER JOIN usertbl u ON u.userid=ss.parent_id
		    WHERE ss.evening_route_id=$route_id AND ss.status=2");
			}
		}
	...
	public function stopstart_alert($route_id, $stop_id, $checkid, $shift_type)
	{
		...
		WHERE ss.route_id=$route_id AND ss.stop_id=$stop_id AND ss.status=2");
				...
				$query = $this->db->query("SELECT ss.*,r.route_name FROM sch_student ss INNER JOIN sch_routestbl r ON r.route_id=ss.route_id INNER JOIN usertbl u ON u.userid=ss.parent_id
		    WHERE ss.evening_route_id=$route_id AND ss.evening_stop_id=$stop_id AND ss.status=2");
			}
		} else if ($checkid == 3) {
		...
		WHERE ss.route_id=$route_id AND ss.stop_id=$stop_id AND (ss.status=2 OR ss.status=3)");
				...
				$query = $this->db->query("SELECT ss.*,r.route_name FROM sch_student ss INNER JOIN sch_routestbl r ON r.route_id=ss.route_id INNER JOIN usertbl u ON u.userid=ss.parent_id
		    WHERE ss.evening_route_id=$route_id AND ss.evening_stop_id=$stop_id AND (ss.status=2 OR ss.status=3)");
			}
		}
		if ($query->num_rows() > 0) {
			return $query->result();
