...
class Api extends REST_Controller
{
	...
	public function mapview_get()
	{
		...
		if ($result['status'] == 1) {
			...
			$student_details = $this->api_model->student_details($userid);
			
		//	$liveroute_details = $this->api_model->livetrackvehiclelist($userid);
			foreach ($student_details as $list_data) {
				$route_ids = array_slice($list_data, 0, 2);
				foreach ($route_ids as $key => $value) {
					$shift_type = $this->api_model->get_shifttype($value);
					$route_id = $value;
					$stop_id = ($shift_type==1) ? $list_data['stop_id'] : $list_data['evening_stop_id'];
					if($shift_type==1)
					$list = $this->api_model->single_livetrackvehicles($route_id);
					else
					$list = $this->api_model->single_livetrackvehicles($route_id);
					...
					if ($route_exit_time) {
						...
						$duration = $this->GetDuration($vehicle_lng, $vehicle_lat, $route_endlng, $route_endlat);
						...
						$class_section_name = $this->api_model->class_section_name($list_data['class_id'], $list_data['section_id']);

						$livedata = array(
							'student_name' => $list_data['student_name'],
							'rollno' => $list_data['student_rollno'],
						...
						$livestop_details = $this->api_model->livestop_details($route_id, $stop_id);
						...
						foreach ($livestop_details as $stoplist) {
						    ...
						    $duration_stop = $this->GetDuration($vehicle_lng, $vehicle_lat, $stoplng, $stop_lat);

							// $expecttime = $distance / 40;
							// $stop_ept_time = floor($expecttime * 60);
							$add_eta_stop = '+ ' . $duration_stop . ' Minutes';
							$add_eta1_stop = $duration_stop . ' Mins';
							$stop_ept_time = date('Y-m-d H:i:s', strtotime($add_eta_stop));
							...
							$livestop_data[] = array(
								...
								'stopreach_ETA_minutes' => $add_eta1_stop,
	...
	public function vehiclecount_get()
	{
		...
		if ($result['status'] == 1) {
			...
			if ($roleid == "15") {
				$data['status'] = 1;
				$data['allvehicle'] = $this->api_model->total_count($userid, $client_id, $roleid,$timezone_hours);
				$this->response($data, REST_Controller::HTTP_OK);
			} else {
				$data['status'] = 1;
				$data['allvehicle'] = array($this->api_model->total_count($userid, $client_id, $roleid,$timezone_hours));
				$this->response($data, REST_Controller::HTTP_OK);
			}
		} else {
			$this->response($result);
	...
	public function vehicledetails_get()
	{
		...
		if ($valid_status) {
			...
			if ($result['status'] == 1) {
				...
				$client_id = $result['data']->client_id;
			    $timezone_hours =  $result['data']->timezone_hours;
				$data['vehicle_details'] = $this->api_model->vehicledetails($deviceimei,$timezone_hours);
	...
	public function vehiclelist_get()
	{
		...
		if ($valid_status) {
			...
			if ($result['status'] == 1) {
				...
				$data['allvehicle'] = $this->api_model->vehiclelist($client_id, $status,$timezone_hours);
	...
	public function genric_parking_post()
	{
		...
		if ($valid_status) {
			...
			if ($result['status'] == 1) {
				...
				$client_id = $result['data']->client_id;
				$timezone_hours = $result['data']->timezone_hours;
				$vehicle_data['vehicle_details'] = $this->api_model->vehicledetails($deviceimei,$timezone_hours);
	...
	public function history_details_admin_POST()
	{
		...
		if ($result['status'] == 1) {
			...
			if ($reporttype != 1) {
				...
				$data = $this->api_model->history_alldata_admin($vehicle, $from_date, $to_date, $client_id, $userid, $role);
				//--------------------------------Its Contain get Values And Store A Variable--------------------------------------------------
				$total_km = round($data->distance);
				$total_park = $data->parking_duration;
				$total_ac = $data->ac_duration;
	...
	public function logout_post()
	{
		...
		if ($result['status'] == 1) {
			$userid = $result['data']->userid;
			$push_code = $this->input->post('push_code');
			$query = $this->db->query("DELETE FROM sch_user_pushcode WHERE userid=$userid AND push_code ='" . $push_code . "' ");
			if ($query) {
				$data['status'] = 1;
...
// 	}
	...
	// 				$data['playback_data'] = $this->api_model->playback_data($deviceimei, $from_date, $to_date, $client_id);
	...
	//public function GetDuration1_get($latitude1 = '12.790639',$longitude1 = '80.221853',$latitude2 = '12.998214',$longitude2 = '80.256070')
	public function GetDuration1_get($latitude1 = '80.221853',$longitude1 = '12.790639',$latitude2 = '80.256070',$longitude2 = '12.998214')
	{ 
		//
	...
	//	echo $url;exit;
		...
		$eta = json_encode(['eta' => $duration]);
		$this->response($eta);
	   
	}
	

	public function trip_start_whatsapp(){
...
// 	public function GetDuration2_get()
// 	{
	
// 	$apiKey = 'pk.eyJ1Ijoic3VyeWExNjMiLCJhIjoiY2xqaDY3bXVqMDVtejNxcHd1bDY1dXhpNCJ9.QuJzGq6Ltkv2tof5yISw_Q';
// 	// $startPoint = '12.790639,80.221853';
// 	$startPoint = '80.221853,12.790639';
// 	// $endPoint = '12.998214,80.256070';
// 	$endPoint = '80.256070,12.998214';
	
// 	$url = "https://api.mapbox.com/directions/v5/mapbox/driving/$startPoint;$endPoint?access_token=$apiKey";
...
// 	// $this->response($data);
// 	if ($data['code'] === 'Ok') {
// 	  $eta = $data['routes'][0]['duration'] / 60; // ETA in minutes
// 	  echo "ETA: $eta minutes";
// 	} else {
// 	  echo "Error calculating ETA: {$data['message']}";
// 	}
