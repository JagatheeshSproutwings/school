...
<div class="col-12">
    ...
    <!-- END: Content-->
<input type="hidden" id="vehicle_imei" value="<?= $vehiclesimei; ?>">
<input type="hidden" id="vehicle_id" value="<?= $vehicleID; ?>">
...
$.ajax({ 
                        type: 'POST',
                        datatype: "json",
                        url: '<?php echo site_url('/dashboard/speed_data'); ?>',
...
//                             $("#chart").empty();
    ...
    //      {
    //     x: new Date('2022-05-10 07:00:00').getTime(),
    //     x2: new Date('2022-05-10 08:00:00').getTime(),
    //     fillColor: '#B3F7CA',
    //     label: {
    //       text: 'ON / OFF'
    //     }
...
//  console.log(obj.ignition);
 var ignition_data =obj.ignition;
 var speed_data =obj.speed_data;

 ignition_data.forEach((ignition1, index) => {
 ignition.push({ 
     x: new Date(ignition1.start_day).getTime(), 
     x2:new Date(ignition1.end_day).getTime(),
     fillColor: '#ccbe21',
     label:{text:'Ignition ON/OFF'}});
  });
//   console.log(ignition);
 ...
 // const date1 = modified_data[index];
  speed.push({ 
      x: new Date(speed1.modified_date), 
      y:parseInt(speed1.speed)});
  });
...
mixed_line_column_area_chart=new ApexCharts(document.querySelector("#chartline"),mixedLineColumnAreaChart);
        ...
        var ed = eda1[1] + '/' + eda1[0] + '/' + eda1[2]+ ' '+eda[1]+' '+eda[2];
           $.ajax({
                   ...
                   } else
                   {
                       ...
                       console.log('obj');
                       alert('No data Found');
                       window.location.replace("<?= base_url(); ?>Dashboard/graph_map_analysis");
                       for (var i = 0; i < parkmarkerss.length; i++) {
